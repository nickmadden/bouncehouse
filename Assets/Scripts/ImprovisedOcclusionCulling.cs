...
public class ImprovisedOcclusionCulling : MonoBehaviour {
	...
	void Start ()
	{
		...
		cam.farClipPlane = defaultFarPlane;
	...
	IEnumerator AdjustFarPlane ()
	{
		while (true)
		{
			int farPlane = (int) cam.farClipPlane + farPlaneBuffer;
			...
			{
				cam.farClipPlane -= rateOfReceding;
				if (cam.farClipPlane < minDistance) cam.farClipPlane = minDistance;
			}
			else
			{
